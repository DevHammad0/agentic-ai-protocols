...
async def main():
    async with MCPServerStreamableHttp(params=mcp_params, name="MySharedMCPServerClient", tool_filter=tool_filter) as mcp_server_client:
        ...
        try:
            ...
            print(f"[INFO] Attempting to explicitly list tools from '{mcp_server_client.name}'...")
            tools = await mcp_server_client.list_tools(run_context=object(), agent=object())
            print(f"\n[INFO] Tools: {len(tools)}")
            print(f"\n[INFO] Names: {[tool.name for tool in tools]}")

            print("\n\n[INFO] Running a simple agent interaction...")
            query_1 = "Call greet_from_shared_server for Junaid"
            query_2 = "Call mood_from_shared_server for Junaid"
            result = await Runner.run(assistant, query_1)
            print(f"\n\n[AGENT RESPONSE]: {result.final_output}")

        except Exception as e:
            print(f"[ERROR] An error occurred during agent setup or tool listing: {e}")

    print(f"[INFO] MCPServerStreamableHttp client '{mcp_server_client.name}' context exited.")
