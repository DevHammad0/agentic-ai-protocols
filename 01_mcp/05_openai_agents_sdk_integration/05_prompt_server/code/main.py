...
from agents import Agent, AsyncOpenAI, OpenAIChatCompletionsModel, Runner, trace, set_tracing_disabled
...
async def get_instructions_from_prompt(mcp_server: MCPServerStreamableHttp, prompt_name: str, **kwargs) -> str:
    ...
    try:
        ...
        print(f"Prompt result messages is list?: {isinstance(prompt_result.messages, list)}")
...
async def demo_code_review(mcp_server: MCPServerStreamableHttp):
...
async def show_available_prompts(mcp_server: MCPServerStreamableHttp):
